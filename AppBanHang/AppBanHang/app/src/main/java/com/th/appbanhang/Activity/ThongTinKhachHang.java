                ...
                if (ten.length() > 0 && sdt.length() > 0 && email.length()> 0)
                {
                    ...
                    StringRequest stringRequest = new StringRequest(Request.Method.POST, Server.Duongdandonhang, new Response.Listener<String>() {
                        ...
                        public void onResponse(final String madonhang) {
                            final String ma = madonhang.trim();
                            Log.d("madonhang",ma);
                            if(Integer.parseInt(ma) >0)
                            {
                                ...
                                StringRequest request = new StringRequest(Request.Method.POST, Server.Duongdanchitietdonhang, new Response.Listener<String>() {
                                    ...
                                    public void onResponse(String response) {
                                        if(response.equals("1"))
                                        {
                                           ...
                                           CheckConnection.ShowToast_Short(getApplicationContext(),"Ban them thanh cong");
                                           Intent intent = new Intent(getApplicationContext(),MainActivity.class);
                                           startActivity(intent);
                                           CheckConnection.ShowToast_Short(getApplicationContext(),"Moi tiep tuc mua hang");
                                        }else
                                        {
                                            CheckConnection.ShowToast_Short(getApplicationContext(),"Du lieu gio hang loi");
                                        }
